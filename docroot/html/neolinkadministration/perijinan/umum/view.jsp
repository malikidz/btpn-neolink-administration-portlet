...
<liferay-ui:header title="perijinan-umum"></liferay-ui:header>
...
<%
	...
	filterURL.setParameter("tabs2", Constants.TAB_PERIJINAN_UMUM);
...
<aui:form action="<%= filterURL %>" method="post">
	<aui:fieldset>
		...
		<aui:input checked="<%= FormUtils.isSelected(false) %>" name="show-complete-active" type="checkbox" />
		<aui:button-row>
...
<liferay-ui:search-container iteratorURL="<%= portletURL %>"
	...
	<%
	List<PerijinanUmum> tempResults = (List<PerijinanUmum>)renderRequest.getAttribute("perijinanumums");
	...
	<liferay-ui:search-container-row
		className="com.codevergence.neolink.admin.model.PerijinanUmum"
		keyProperty="id" 
		modelVar="model">
		...
		<liferay-ui:search-container-column-text name="berakhir-dokumen-date" property="tglBerakhirDokumen" />
		<liferay-ui:search-container-column-text name="active" value='<%= model.isActive() ? "active" : "inactive" %>' />

		<liferay-ui:search-container-column-jsp path="/html/neolinkadministration/perijinan/umum/actions.jsp" align="right" />
	</liferay-ui:search-container-row>
